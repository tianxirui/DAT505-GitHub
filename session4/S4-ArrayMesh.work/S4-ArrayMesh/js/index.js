...
function init() {
  ...
  for (var x = -10; x < 10; x += 5) { // Start from -45 and sequentially add one every 5 pixels
    for (var y = -10; y < 10; y += 5) {
      ...
      if ( x >= 0 && y >= 0 && z >=0 ){
      var boxMaterial = new THREE.MeshLambertMaterial({color: 0xF5F6CA});
    } //ture
      /*else{
        var boxMaterial = new THREE.MeshLambertMaterial({color: 0xFFFFFF});
      }*/
      ...
      var boxMaterial = new THREE.MeshLambertMaterial({color: 0xE4DC8A});
    } //ture
      else if ( x >= 0 && y < 0 && z >= 0 ){
      var boxMaterial = new THREE.MeshLambertMaterial({color: 0x72640C});
    } //ture
      else if ( x < 0 && y < 0 && z >= 0 ){
